...
congenital_line <- data_congenital %>% 
  ...
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci), fill = "purple", alpha = 0.1) +
  xlab("Years") +
  ylab("Rate") +
  scale_x_continuous(expand = c(0, 0), breaks = c(2014:2022), limits = c(2014,2022.1))+
...
```{r}
combined_plot_hiv_curve <- ggplot() +
  geom_line(data = data_congenital, aes(x = year, y = rate), color = "purple", size = 1.2) +
  geom_ribbon(data = data_congenital, aes(x = year, ymin = lower_ci, ymax = upper_ci), fill = "purple", alpha = 0.1) +
  geom_line(data = data_maternal, aes(x = year, y = rate), color = "blue", size = 1.2) +
  geom_ribbon(data = data_maternal, aes(x = year, ymin = lower_ci, ymax = upper_ci), fill = "blue", alpha = 0.1) +
  xlab("Years") +
  ylab("Rate") +
  scale_x_continuous(expand = c(0, 0), breaks = c(2014:2022), limits = c(2014, 2022.1)) +
  ggsci::scale_color_jama(alpha = 0.8) +
  #ggtitle("Tasa de Sífilis Congénita y Materna") +
  theme_bw() +
  theme(
    axis.title = element_text(face = "bold", size = 11),
    legend.text = element_text(size = 8, face = "bold"),
...
print(combined_plot_hiv_curve)
ggsave("figure_curva_syp_cong_mater.png", dpi = 300, width = 10)
```
```{r}
#Combinando dos curvas
tendencias <- plot_grid(
  syphi_tamizaje,combined_plot_hiv_curve,
  ncol = 2
)
tendencias
```
...
dumbell <- plot_grid(
  d15_d22_screening,syphcongenita,syphmaterna,
  ncol = 3
)
dumbell
```
```{r}
#combinando con curvas
dumbell_tendencia <- plot_grid(
  dumbell,tendencias,
  ncol = 1,
  labels = c ("c", "d"),
...
```
```{r}
#combinando con gráfico de gol

total <- plot_grid(
  figure2,dumbell_tendencia,
  ncol = 2,
