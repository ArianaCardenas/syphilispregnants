...
# Para syphilis_year
syphilis_year_dep <- syphilis_year %>%
  mutate(
    lower_ci = syphilis_screening - 1.96 * se,
    upper_ci = syphilis_screening + 1.96 * se
  )

syphilis_year_dep <- syphilis_year_dep %>%
  mutate(
    across(c(syphilis_screening, lower_ci, upper_ci), ~round(. * 100, 1))
  )
```
