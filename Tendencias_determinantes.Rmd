...
poverty_mr %>% 
  mutate(
    across(wealth_indexMiddle:wealth_indexRichest,.fns = ~round(.,3)*100)) %>%
...
```{r}
poverty_mrg =
  ggplot(data = poverty_mr,aes(x = year, y = rate_mat, group = wealth_index))+
  geom_line(size = 1.2, aes(col = wealth_index))+
  xlab("Years") +
  ylab("Prop. %") +
  theme_bw() +
...
```{r}
poverty_crg =
  ggplot(data = poverty_cr,aes(x = year, y = rate_con, group = wealth_index))+
  geom_line(size = 1.2, aes(col = wealth_index))+
  xlab("Years") +
  ylab("Prop. %") +
  theme_bw() +
