...
precare = 
  data_enaho %>%
  left_join(data_congenital_dep, by = c("year","NOMBDEP")) %>%
  select(control_iron,year,NOMBDEP,conglome,estrato,facpob07,rate_con) %>%
  filter(!is.na(control_iron))%>%
  mutate(
    NOMBDEP = as.factor(NOMBDEP),
    year = as.factor(year),
    control_iron = as.factor(control_iron)
  ) 
...
syphilis_precare<- svyby(~control_iron,~NOMBDEP+year,svymean,design=design,deff=TRUE,na.rm=TRUE)
...
datamap <- bi_class(syphilis_precare, x = rate_con, y = control_ironno, style = "quantile", dim = 2)
...
datamap <- bi_class(syphilis_precare15, x = rate_con, y = control_ironno, style = "quantile", dim = 2)
...
datamap <- bi_class(syphilis_precare18, x = rate_con, y = control_ironno, style = "quantile", dim = 2)
...
datamap <- bi_class(syphilis_precare22, x = rate_con, y = control_ironno, style = "quantile", dim = 2)
...
mapprecare <- map | map15 | map18 |map22 | (legend/plot_spacer())
 #+ plot_layout(widths = c(1, 1, 1, 1, 1))
...
maptotals <- finalPlot_poverty | finalPlot_education | finalPlot_precare 
 #+ plot_layout(widths = c(1, 1, 1, 1, 1))
