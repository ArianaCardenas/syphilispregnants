...
mappoverty_CS <- map_poverty | map15 | map18 |map22 | (legend/plot_spacer())
...
mapeducation_CS <- map_education | map15 | map18 |map22 | (legend/plot_spacer())
 #+ plot_layout(widths = c(1, 1, 1, 1, 1))
...
precare = 
  data_enaho %>%
  left_join(data_congenital_dep, by = c("year","NOMBDEP")) %>%
  select(prenatal_visit,year,NOMBDEP,conglome,estrato,facpob07,rate_con) %>%
...
datamap <- bi_class(syphilis_precare, x = rate_con, y = prenatal_visitno, style = "quantile", dim = 2)
...
datamap <- bi_class(syphilis_precare15, x = rate_con, y = prenatal_visitno, style = "quantile", dim = 2)
...
datamap <- bi_class(syphilis_precare18, x = rate_con, y = prenatal_visitno, style = "quantile", dim = 2)
...
datamap <- bi_class(syphilis_precare22, x = rate_con, y = prenatal_visitno, style = "quantile", dim = 2)
...
mapprecare_CS <- map_precare | map15 | map18 |map22 | (legend/plot_spacer())
 #+ plot_layout(widths = c(1, 1, 1, 1, 1))
...
maptotals <- (map_poverty | legend_poverty | map_education | legend_education | map_precare | legend_precare) + plot_layout(widths = c(1, 0.6, 1, 0.6, 1, 0.6)) 
...
```{r}
# ggsave("Figures/Bivariate_congenital.png", plot = maptotals, width = 10, height = 4, dpi = 300)
```
