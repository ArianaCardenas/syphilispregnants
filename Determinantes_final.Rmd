...
# Prenatal visit 
```{r}
table(data_enaho$control_iron,data_enaho$year,data_enaho$visit_preg)
table(data_enaho$control_iron,data_enaho$year)
table(data_enaho$control_iron,data_enaho$visit_preg)
```
...
no_pre = 
  ...
  left_join(data_maternal_prov, by = c("year","NOMBDEP","NOMBPROV")) %>%
  left_join(data_congenital_prov, by = c("year","NOMBDEP","NOMBPROV")) %>%
  select(visit_preg, control_iron, year, region_natural, NOMBDEP,NOMBPROV,conglome,estrato,facpob07,rate_mat,rate_con) %>%
...
no_pre_mr = no_pre_mr %>%  
  mutate(
    ...
    no_pre_cod = case_when(no_pre <55.28 ~ "First tercile",
                            no_pre >=55.28 & no_pre <81.38 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
...
no_pre_cr = no_pre_cr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_pre <57.53 ~ "First tercile",
                            no_pre >=57.53 & no_pre <79.55 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
  summarize(
...
no_pre_mr = no_pre_mr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_pre <65.74 ~ "First tercile",
                             no_pre >=65.74 & no_pre <85.34 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
    summarize(
...
no_pre_cr = no_pre_cr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_pre <68.02 ~ "First tercile",
                             no_pre >=68.02 & no_pre <83.57 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
    summarize(
...
no_pre_mr = no_pre_mr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_pre <49.89 ~ "First tercile",
                             no_pre >=49.89 & no_pre <79.45 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
    summarize(
...
no_pre_cr = no_pre_cr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_pre <55.64 ~ "First tercile",
                             no_pre >=55.64 & no_pre <74.85 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
    summarize(
...
no_pre_mr = no_pre_mr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_pre <45.02 ~ "First tercile",
                             no_pre >=45.02 & no_pre <66.07 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
    summarize(
...
no_pre_cr = no_pre_cr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_pre <48.88 ~ "First tercile",
                             no_pre >=48.88 & no_pre <67.46 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
    summarize(
