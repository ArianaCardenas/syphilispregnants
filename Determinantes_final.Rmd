...
# Prenatal visit 
```{r}
table(data_enaho$visit_preg,data_enaho$year)
```
...
no_pre = 
  ...
  left_join(data_maternal_prov, by = c("year","NOMBDEP","NOMBPROV")) %>%
  left_join(data_congenital_prov, by = c("year","NOMBDEP","NOMBPROV")) %>%
  select(visit_preg, year, region_natural, NOMBDEP,NOMBPROV,conglome,estrato,facpob07,rate_mat,rate_con) %>%
  filter(!is.na(visit_preg))%>%
  filter(year > "2014") %>%
  mutate(
    NOMBPROV = as.factor(NOMBPROV),
    year = as.factor(year),
    no_pre = as.factor(visit_preg)
  ) %>%
  group_by(year) %>%
  nest() %>%
  mutate(
...
no_pre_mr = no_pre_mr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_preno <94.93 ~ "First tercile",
                            no_preno >=94.93 & no_preno <97.21 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
...
no_pre_cr = no_pre_cr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_preno <94.84 ~ "First tercile",
                            no_preno >=94.84 & no_preno <96.63 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
  summarize(
...
no_pre_mr = no_pre_mr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_preno <95.73 ~ "First tercile",
                             no_preno >=95.73 & no_preno <97.28 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
    summarize(
...
Prenatal visit objective 3 (terciles)
```{r}
no_pre_cr = 
  no_pre %>%
  group_by(year) %>%
...
no_pre_cr = no_pre_cr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_preno <95.61 ~ "First tercile",
                             no_preno >=95.61 & no_preno <96.73 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
    summarize(
...
```{r}
Costa_obj3 =
  ggplot(data = no_pre_cr,aes(x = year, y = rate_con_mean, group = no_pred_cod))+
  geom_line(size = 1.2, aes(col = no_pre_cod))+
  geom_line(aes(y=objective),color = "black", linetype = "dashed", size=1.2) +
  xlab("Years") +
  ylab("Rate") +
...
no_pre_mr = no_pre_mr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_preno <95.22 ~ "First tercile",
                             no_preno >=95.22 & no_preno <97.61 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
    summarize(
...
no_pre_cr = no_pre_cr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_preno <94.96 ~ "First tercile",
                             no_preno >=94.96 & no_preno <97.20 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
    summarize(
...
Selva Prenatal visit objective 2 (terciles)
```{r}
no_pre_mr = 
  no_pre %>%
  group_by(year) %>%
...
no_pre_mr = no_pre_mr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_preno <92.59 ~ "First tercile",
                             no_preno >=92.59 & no_preno <95.50 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
    summarize(
...
Selva Prenatal visit objective 3 (terciles)
```{r}
no_pre_cr = 
  no_pre %>%
  group_by(year) %>%
...
no_pre_cr = no_pre_cr %>%  
  mutate(
    no_pre = no_preno*100,
    no_pre_cod = case_when(no_preno <92.45 ~ "First tercile",
                             no_preno >=92.45 & no_preno <94.87 ~ "Second tercile",
                            TRUE ~ "Third tercile")) %>%
  filter(!is.na(no_pre_cod))%>%
  group_by(year,no_pre_cod)%>%
    summarize(
...
Prenatal_graph
ggsave("No_prenatal_visit.png", plot = Prenatal_graph)
```