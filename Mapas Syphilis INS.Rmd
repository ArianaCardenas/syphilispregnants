...
Se eliminan las provincias con menos de diez nacidos vivos por año. La lista es la sgte:
```{r}
provmenosde10 <- data_maternal_table %>%
  filter(n<11)
```

```{r, echo=FALSE}

data_maternal_table <- data_ins_long %>%
...
data_maternal_table <- data_maternal_table %>%
...
```{r}
ggplot() +
  geom_sf(data = data_total_reactivos, aes(geometry = geometry,fill = rate),
