...
data_mapas <- data_mapas %>%
  left_join(departamentos, by = c("CCDD","NOMBDEP"))
  #left_join(provincias, by = c("CCPP","CCDD","NOMBDEP","NOMBPROV"))

rm(data_ubigeo,data_syphilis,data_ubigeo_macroregion,provincias,departamentos)
```

```{r}
datos_filtrados <- data_mapas %>%
  filter(syphilis_screening == "Yes", year == 2010) %>%
  #filter(syphilis_screening == "Yes", !is.na(Macrozona), year == 2010) %>%
  group_by(NOMBDEP, year, geometry) %>%
  mutate(
